...
import com.ppc.simplejson.exeption.JsonParseException;
...
public class Tokenizer {
    ...
    private void tokenize() throws IOException {
        ...
        do {
            token = start();
            tokenList.add(token);
        } while (token.getTokenType() != TokenTypeEnum.END_DOCUMENT);
    }
    ...
    private Token start() throws IOException {
        ...
        for (;;) {
            if (!charReader.hasMore()) {
                return new Token(TokenTypeEnum.END_DOCUMENT, null);
            }
        ...
        switch (ch) {
            case '{':
                return new Token(TokenTypeEnum.BEGIN_OBJECT, String.valueOf(ch));
            case '}':
                return new Token(TokenTypeEnum.END_OBJECT, String.valueOf(ch));
            case '[':
                return new Token(TokenTypeEnum.BEGIN_ARRAY, String.valueOf(ch));
            case ']':
                return new Token(TokenTypeEnum.END_ARRAY, String.valueOf(ch));
            case ',':
                return new Token(TokenTypeEnum.SEP_COMMA, String.valueOf(ch));
            case ':':
                return new Token(TokenTypeEnum.SEP_COLON, String.valueOf(ch));
            case 'n':
                return readNull();
            case 't':
            case 'f':
    ...
    private boolean isDigitOne2Nie(char ch) {
        return ch >= '1' && ch <= '9';
    }
